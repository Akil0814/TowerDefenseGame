...
public:
	Enemy()
	{
		...
		timer_sketch.set_on_timeout([&]() {speed = max_speed; });
	}
	...
	void on_update(double delta)
	{
		...
		Vector2 move_distance = velocity * delta;//一次更新可以移动的距离
		Vector2 target_distance = position_target - position;//目标距离
		position += move_distance < target_distance ? move_distance : target_distance;//移动距离是否会超过目标距离
		...
		{
			if (is_show_x_amin)
				animation_current = velocity.x > 0 ? &anim_right_sketch : &anim_left_sketch;
			else
				animation_current = velocity.y > 0 ? &anim_down_sketch : &anim_up_sketch;
		}
		else
		{
			if (is_show_x_amin)
				animation_current = velocity.x > 0 ? &anim_right : &anim_left;
			else
				animation_current = velocity.y > 0 ? &anim_down : &anim_up;
		}
	...
	void on_render(SDL_Renderer* renderer)
	{
		...
		animation_current->on_render(renderer, point);
	...
	double get_route_process()const
	{
		...
		return (double)idx_target / (route->get_idx_list().size() - 1);//返回行进路程的进度
	}
